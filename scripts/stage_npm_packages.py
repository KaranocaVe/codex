...
CLI_NPM_NAME = getattr(_BUILD_MODULE, "CODEX_NPM_NAME", "wildex").split("/")[-1]
...
def tarball_name_for_package(package: str, version: str) -> str:
    if package in CODEX_PLATFORM_PACKAGES:
        platform = package.removeprefix(f"{CLI_NPM_NAME}-")
        return f"{CLI_NPM_NAME}-npm-{platform}-{version}.tgz"
